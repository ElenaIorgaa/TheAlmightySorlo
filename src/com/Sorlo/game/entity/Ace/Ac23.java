...
public class Ac23 extends Entity {
    ...
    public Ac23(Sprite16 sprite, Vector2f origin, int size, ENEM e1, PlayerNivel2 playerNivel2) {
        ...
        proiectilVerde=new ProiectilVerde2(new Sprite32("entity/PAlbastru.png"),new Vector2f(188*16,18*16),32,m1);
            ...
            if (m1.isAlive) {
                if (!m1.killsPlayer(207*16)) {
                    ...
                    if (n1.attack) {
                        ...
                        if (pos.x >= 195*16 && pos.x <=195*16+32*2+16) {
                            dx += 0.1;
                            m1.setInc(300);
                        } else {
                            dx -= 0.1;
    ...
    public boolean isGalbenu(int pozx) {
        if (pozx >= 194*16+32 && pozx < 194*16+32+32)
            return true;
        else return false;
    }

    public boolean isRosuu(int pozx) {
        if (pozx >= 194*16 && pozx < 194*16+32)
            return true;
        else return false;
    }

    public boolean isRozu(int pozx) {
        if (pozx >= 194*16+32+32 && pozx < 194*16+32+32+32)
            return true;
        else return false;
    }

    public boolean isAlbastruu(int pozx) {
        if (pozx >= 194*16+32+32+32 && pozx < 194*16+32+32+32+32)
            return true;
        else return false;
    }

    public boolean isMovu(int pozx) {
        if (pozx >= 194*16+32*4 && pozx < 194*16+32*5)
            return true;
        else return false;
    }

    public boolean isVerdeu(int pozx) {
        if (pozx >= 194*16+32*5 && pozx < 194*16+32*6)
            return true;
        else return false;
    }
            ...
            if (isAlbastruu(pozx)) {
                {
                    ...
                    dx = 0;
                    pos.x = 195*16;
                    proiectilVerde.render(g,195*16);
                    System.out.println("Ac: "+pos.x+","+pos.y+"*****************************************************************");
                }

            } else if (isRosuu(pozx) || isVerdeu(pozx) || isGalbenu(pozx) || isMovu(pozx) || isRozu(pozx)) {
