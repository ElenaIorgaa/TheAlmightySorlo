...
public class Ac26 extends Entity {
    ...
    public Ac26(Sprite16 sprite, Vector2f origin, int size, ENEM e1, PlayerNivel2 playerNivel2) {
        ...
        proiectilVerde=new ProiectilVerde2(new Sprite32("entity/ProiectilVerde.png"),new Vector2f(359*16,7*16),32,m1);
            ...
            if (m1.isAlive) {
                if (!m1.killsPlayer(400*16)) {
                    ...
                    if (n1.attack) {
                        ...
                        if (pos.x >= 367*16 && pos.x <=367*16+32*2+16) {
                            dx += 0.1;
                            m1.setInc(300);
                        } else {
                            dx -= 0.1;
    ...
    public boolean isGalbenu(int pozx) {
        if (pozx >= 366*16+32 && pozx < 366*16+32+32)
            return true;
        else return false;
    }

    public boolean isRosuu(int pozx) {
        if (pozx >= 366*16 && pozx < 366*16+32)
            return true;
        else return false;
    }

    public boolean isRozu(int pozx) {
        if (pozx >= 366*16+32+32 && pozx < 366*16+32+32+32)
            return true;
        else return false;
    }

    public boolean isAlbastruu(int pozx) {
        if (pozx >= 366*16+32+32+32 && pozx < 366*16+32+32+32+32)
            return true;
        else return false;
    }

    public boolean isMovu(int pozx) {
        if (pozx >= 366*16+32*4 && pozx < 366*16+32*5)
            return true;
        else return false;
    }

    public boolean isVerdeu(int pozx) {
        if (pozx >= 366*16+32*5 && pozx < 366*16+32*6)
            return true;
        else return false;
    }
            ...
            if (isVerdeu(pozx)) {
                {
                    ...
                    dx = 0;
                    pos.x = 367*16;
                    proiectilVerde.render(g,367*16);
                    System.out.println("Ac: "+pos.x+","+pos.y+"*****************************************************************");
                }

            } else if (isGalbenu(pozx) || isRosuu(pozx) || isAlbastruu(pozx) || isMovu(pozx) || isRozu(pozx)) {
